...
def convert_kohya_to_peft_keys(kohya_dict: dict, kohya_prefix="", peft_prefix: str = "base_model.model", device="cpu") -> dict:
    peft_dict = {}
    for k, v in kohya_dict.items():
        ...
        new_key = new_key.replace('_individual_token_refiner_blocks_','.individual_token_refiner.blocks.')
...
def load_lora(model, lora_path, device):
    ...
    peft_weights = convert_kohya_to_peft_keys(kohya_weights, kohya_prefix='Hunyuan_video_I2V', device=device)
...
def get_cond_latents(args, latents, vae):
    ...
    first_images_pixel_values = torch.cat(first_images_pixel_values).unsqueeze(0).unsqueeze(2).to(vae.device)
...
def prepare_model_inputs(args,
    ...
    if len(latents.shape) == 1:
        if len(media.shape) == 4:
            ...
            raise ValueError(f"Only support media with shape (b, c, h, w) or (b, c, f, h, w), but got {media.shape}.")
        ...
        latents = latents * vae.config.scaling_factor  # vae cache is not multiplied by scaling_factor
    else:
        raise ValueError(
    ...
    is_uncond = torch.tensor(1).to(torch.int64) if random.random() < args.sematic_cond_drop_p else torch.tensor(
        0).to(torch.int64)
    ...
    text_outputs = text_encoder.encode({"input_ids": text_ids, "attention_mask": text_mask},
                                       data_type=batch_args[-1]['type'][0], semantic_images=semantic_images)
    text_states = text_outputs.hidden_state
    text_mask = text_outputs.attention_mask
    text_states_2 = (
        text_encoder_2.encode({"input_ids": text_ids_2, "attention_mask": text_mask_2},
                              data_type=data_type).hidden_state
    ...
    freqs_cos, freqs_sin = get_rope_freq_from_size(args, model, latents_size, ndim, target_ndim,
                                                   rope_theta_rescale_factor=rope_theta_rescale_factor,
                                                   rope_interpolation_factor=rope_interpolation_factor)

    # ===================================== Pack model kwargs ==================================
    model_kwargs = dict(text_states=text_states,  # [b, 256, 4096]
...
def get_rope_freq_from_size(args, model, latents_size, ndim, target_ndim, rope_theta_rescale_factor=1.0, rope_interpolation_factor=1.0):
